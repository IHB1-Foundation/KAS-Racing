...
router.post('/create', asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const normalizedAddress = normalizeEvmAddress(body.playerAddress);
    if (!normalizedAddress) {
      res.status(400).json({ error: 'playerAddress must be a valid EVM address' });
      return;
    }

    const result = await createMatchLobby(normalizedAddress, body.betAmountWei);
    res.json(result);
  } catch (error) {
...
router.post('/join', asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const result = await joinMatch(body.joinCode, normalizedAddress);
    res.json(result);
  } catch (error) {
    handleMatchError(res, error);
  }
...
router.post('/:id/submit-score', asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    const result = await submitScore(id, normalizedAddress, body.score);
    res.json(result);
  } catch (error) {
    handleMatchError(res, error);
  }
